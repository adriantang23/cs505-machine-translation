...
\begin{abstract}
We present a comparative study of three approaches to French--English machine translation that span the historical arc of the field: a word-based IBM Model 1 system, a phrase-based trigram language model and a log-linear stack decoder, and a pre-trained neural encoder--decoder (Helsinki-NLP OPUS-MT). We implement both statistical systems from scratch and train them on a 50{,}000 sentence-pair slice of the Europarl corpus, then evaluate all three systems on the WMT14 French--English news test set (\texttt{newstest2014}) using SacreBLEU. Our preliminary results show a large quality gap between the statistical and neural approaches: IBM Model 1 achieves \textbf{4.23} BLEU, our phrase-based system achieves \textbf{12.35} BLEU, and OPUS-MT achieves \textbf{37.99} BLEU. Qualitative analysis illustrates characteristic failure modes of each system, including the absence of reordering in IBM Model 1 and the residual fluency and coverage issues of the phrase-based system on out-of-domain news text. We also discuss the risk of train--test contamination for OPUS-MT and describe our plan to address it, along with planned extensions including larger training sets, MERT-style weight tuning, an IBM Model 2 baseline, and a companion re-evaluation of the phrase-based system on a held-out in-domain slice of the Europarl corpus following the evaluation protocol of \citet{koehn-etal-2003-statistical}.
\end{abstract}
...
\paragraph{(1) Model-card inspection.} We consulted the Helsinki-NLP \texttt{opus-mt-fr-en} model card and associated OPUS-MT training recipes to determine which corpora were used and whether WMT news test sets were held out. 
 
\paragraph{(2) Overlap check against our training slice.} For our from-scratch systems we can directly verify no contamination: we hash every sentence in \texttt{newstest2014} and check membership against the 50{,}000-pair Europarl training slice.
...
Table~\ref{tab:bleu} shows corpus-level BLEU scores for the three systems on \texttt{newstest2014}. The phrase-based system outperforms IBM Model 1 by roughly 8 BLEU points, consistent with the well-known benefit of modeling multi-word units, lexical weighting, and target-side fluency. The neural baseline still outperforms both statistical systems by a wide margin, reflecting the combined effect of substantially more training data, domain-balanced training, and a much richer model class. As a sanity check, the same phrase-based system reaches \textbf{23.79} BLEU on the Koehn-style held-out in-domain Europarl test set of 1{,}755 short sentences, broadly consistent with the numbers reported by \citet{koehn-etal-2003-statistical} for comparable training sizes, confirming that our implementation of Moses-style phrase extraction, lexical weighting, trigram language modeling, and log-linear stack decoding is functioning as intended.
...
\paragraph{Full in-domain Europarl re-evaluation.} Our current \texttt{newstest2014} comparison pays a substantial domain-shift cost for the statistical systems. For the final report we will carry out a full companion evaluation in which the test set is drawn from the Europarl corpus itself (the sanity-check number of 23.79 BLEU reported above is a first pass at this setup), following the Koehn-style protocol of \citet{koehn-etal-2003-statistical}. This will separate the model-class gap from the domain-shift gap and give us an in-domain baseline for both IBM Model 1 and the phrase-based system.
